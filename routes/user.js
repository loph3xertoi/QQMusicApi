...
const user = {
  ...
  '/refresh': async ({ req, res, request }) => {
    ...
    let url = `https://u.y.qq.com/cgi-bin/musics.fcg?sign=${sign}&format=json&inCharset=utf8&outCharset=utf-8&data=${encodeURIComponent(
      JSON.stringify(data)
    )}`;
  ...
  '/applyCookie': ({ req, res, globalCookie }) => {
    const { id } = req.query;
    if (!id) {
      return res.send({
        result: 500,
  ...
  '/setCookie': ({ req, res, globalCookie }) => {
    ...
    jsonFile.writeFile('data/rawCookie.json', globalCookie.rawCookie);
  ...
  // 获取用户收藏的歌单
  '/collect/playlist': async ({ req, res, request }) => {
